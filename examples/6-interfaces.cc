...
namespace
{
	...
	// TODO(andreyka): Welcome to your new test for Ltm. You're almost
	// done, but first:
	...
	TEST(LtmTest, DoesFoo)
	{
		EXPECT_THAT(1, Eq(2));  // Obvious failure, for educational purposes only.
	}

	// EXAMPLE: With a test fixture and TEST_Fs
	// A test fixture is used because {TODO(andreyka): put reason here}.
	class LtmTest : public Test
	{
	protected:
		...
		// See http://go/gunitprimer#Test_Fixtures_Using_the_Same_Dat for details.
	};

	TEST_F(LtmTest, HasPropertyBar)
	{
		EXPECT_THAT(1, Eq(2));  // Obvious failure, for educational purposes only.
	}
	// End TEST_F EXAMPLE.
